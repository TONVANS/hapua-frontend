...
import { Hotel as HotelIcon, Plus, Search, RefreshCw, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';

interface HotelsHeaderBarProps {
  search: string;
  setSearch: (val: string) => void;
  selectedStarFilter: string;
  setSelectedStarFilter: (val: string) => void;
  loading: boolean;
  onRefresh: () => void;
  onOpenCreate: () => void;
}

export function HotelsHeaderBar({
  search,
  setSearch,
  selectedStarFilter,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-[#e2e8f0] shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-bold text-[#002660] tracking-tight">Hotel Accommodations</h2>
          </div>
          <p className="text-xs text-[#4f616f] mt-1 ml-10">
            Official partner hotels, luxury resorts, and accommodation guides for council delegates
          </p>
        </div>
      ...
      <div className="p-4 bg-white rounded-2xl border border-[#e2e8f0] shadow-xs flex flex-col md:flex-row gap-3 items-center justify-between">
        <div className="relative w-full md:w-80">
          ...
          <Input
            placeholder="Search hotel name, address, or contact..."
            value={search}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          <div className="flex items-center gap-2">
            <Label className="text-xs font-semibold text-[#4f616f] whitespace-nowrap">Star Tier:</Label>
            <select
            ...
            >
              ...
              <option value="4">4-Star Superior</option>
